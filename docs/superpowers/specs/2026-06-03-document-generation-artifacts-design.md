...
Add agent-assisted document generation to Slop, modeled after AnythingLLM's separation between
...
file. Slop saves the generated file into the user's Artifacts area and renders a download card in
the chat.
...
re-rendered from chat history. Slop should follow the product shape but adapt the storage and access
model to Slop's per-user volume and SSE chat stream.
...
When the model chooses a document-generation tool, Slop streams a tool status event, creates the
...
Slop also stores an internal artifact id and exact relative path so downloads never trust a raw path
from the browser.
...
uses Slop's embedded Go font, and does not yet provide robust word wrapping, table layout, or
...
are local Slop capabilities, need access to the authenticated user and current thread/project scope,
...
paths, `..`, reserved `.slop` paths, symlink escapes, empty filenames, and filenames that normalize
to unsafe names. Generated filenames are collision-safe; if a display filename already exists, Slop
...
Add an artifact card component inside chat history. It should use Slop's existing Warm Editorial
...
atomic: write to `.slop/tmp`, fsync where practical, then rename into `outputs/`.
...
- rejection of traversal, absolute paths, `.slop`, and symlink escape
