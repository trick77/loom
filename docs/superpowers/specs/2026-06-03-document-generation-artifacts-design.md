...
Add agent-assisted document generation to Spark, modeled after AnythingLLM's separation between
...
file. Spark saves the generated file into the user's Artifacts area and renders a download card in
the chat.
...
re-rendered from chat history. Spark should follow the product shape but adapt the storage and access
model to Spark's per-user volume and SSE chat stream.
...
When the model chooses a document-generation tool, Spark streams a tool status event, creates the
...
Spark also stores an internal artifact id and exact relative path so downloads never trust a raw path
from the browser.
...
paths, `..`, reserved `.spark` paths, symlink escapes, empty filenames, and filenames that normalize
to unsafe names. Generated filenames are collision-safe; if a display filename already exists, Spark
...
Add an artifact card component inside chat history. It should use Spark's existing Warm Editorial
...
atomic: write to `.spark/tmp`, fsync where practical, then rename into `outputs/`.
...
- rejection of traversal, absolute paths, `.spark`, and symlink escape
