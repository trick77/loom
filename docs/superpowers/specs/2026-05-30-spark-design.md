...
- **slopr** — Go binary (API + static assets), **lean image** (no node/python runtimes). Mounts
...
- **MCP as separate containers:** the slopr image stays lean (no foreign runtimes); MCP servers run as
...
│   └─ …
├─ projects/             # slopr-managed, one folder per project (keyed to project id)
│   └─ <project-id>/     # RAG scope: this project
│       ├─ …
│       └─ outputs/      # thread results for this project
└─ .slopr/                 # reserved, NOT shown in the Artifacts browser: upload staging (Tika hotdir), tmp
```

- The Artifacts browser shows only `files/` + `projects/<id>/`; `.slopr/` stays hidden.
...
```
┌──────────────┬───────────────────────────────────┬─────────────────┐
│  slopr         │  Thread title            [Artifacts]│  Context panel  │
│  + New chat  │ ───────────────────────────────────│  (switchable)   │
│  🔎 Search   │  ▸ User: ...                        │                 │
│              │  ▸ Assistant: ...                   │  • Sources      │
