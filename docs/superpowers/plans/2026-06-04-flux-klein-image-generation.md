...
**Goal:** Add first-class text-to-image generation to Spark using the BFL FLUX.2 [klein] API, saving generated images as normal per-user/project artifacts and rendering them inline in chat.
...
**Tech Stack:** Go stdlib `net/http`, BFL API `POST https://api.bfl.ai/v1/flux-2-klein-4b` with `x-key`, existing Spark SSE/tool/artifact pipeline, React + TypeScript.
...
- **Modify** `backend/cmd/spark/main.go` — instantiate BFL client/tool when configured.
...
SPARK_BFL_BASE_URL=https://api.bfl.ai/v1
SPARK_BFL_API_KEY=
SPARK_BFL_MODEL=flux-2-klein-4b
```
...
In `backend/internal/httpapi/server.go`, import `github.com/trick77/spark/internal/imagegen`.
...
In `backend/internal/httpapi/message_stream_handlers.go`, import `github.com/trick77/spark/internal/imagegen` if needed by helper signatures.
...
In `backend/cmd/spark/main.go`, add:
...
Expected: PASS and `bin/spark` exists.
...
- [ ] **Step 4: Run Spark with image generation enabled**
...
SPARK_AUTH_MODE=dev \
SPARK_ADDR=127.0.0.1:18081 \
SPARK_PUBLIC_URL=http://localhost:18081 \
SPARK_DB_PATH=/private/tmp/spark-imagegen-dev.db \
SPARK_BFL_API_KEY="$BFL_API_KEY" \
SPARK_BFL_MODEL=flux-2-klein-4b \
./bin/spark
```
