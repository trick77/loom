...
**Goal:** Replace Spark's split waiting indicators with one persistent animated `Thinking` block during active assistant turns.
...
.spark-thinking-label-active::after {
  position: absolute;
  inset: 0;
  ...
  opacity: 0;
  animation: spark-thinking-sweep 4.5s linear infinite;
}
```
