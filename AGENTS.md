...
- The model endpoints are llmwire's: `LLMWIRE_MIMO_API_KEY` (chat) and `LLMWIRE_OPENAI_API_KEY`
  ...
  A set key turns the capability on. `LLMWIRE_EMULATE_OPENCODE=true` presents the chat calls as the
  opencode client, read by llmwire (its MiMo provider entry is marked for it regardless).
