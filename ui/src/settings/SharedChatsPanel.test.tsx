...
describe("SharedChatsPanel", () => {
  ...
  it("lists active and disabled shares with the right per-row controls", async () => {
    ...
    expect(
      screen.getAllByRole("button", { name: "Disable link" }),
    ).toHaveLength(1);
    expect(
      screen.queryByText("You haven’t shared any chats yet."),
  ...
  it("shows an empty-state notice when nothing has been shared", async () => {
    ...
    expect(
      await screen.findByText("You haven’t shared any chats yet."),
    ).toBeInTheDocument();
  });
  ...
  it("shows an error message when the list fails to load", async () => {
    ...
    expect(
      await screen.findByText("Failed to load shared chats."),
    ).toBeInTheDocument();
  });
  ...
  it("revokes a share and flips the row to disabled", async () => {
    ...
    fireEvent.click(
      await screen.findByRole("button", { name: "Disable link" }),
    );
  ...
  it("surfaces an error when revoking fails and keeps the row active", async () => {
    ...
    expect(
      await screen.findByText("Failed to disable the link."),
    ).toBeInTheDocument();
    expect(
  ...
  it("copies a relative share URL as an absolute one and confirms", async () => {
    ...
    expect(writeText).toHaveBeenCalledWith(
      `${window.location.origin}/share/tok1`,
    );
    expect(
