...
const memory = { content: "remembered things", updatedAt: "2026-06-01T00:00:00Z" };
...
describe("listProjects", () => {
  ...
  test.each([true, false])("sends archived=%s when specified", async (archived) => {
    const fetchMock = stubFetch(Response.json([]));

    await listProjects(archived);

    expect(fetchMock).toHaveBeenCalledWith(`/api/projects?archived=${String(archived)}`);
  });
...
describe("createProject", () => {
  test("posts the project payload", async () => {
    ...
    await expect(createProject({ name: "Roadmap", description: "Planning" })).resolves.toEqual(project);
    expect(fetchMock).toHaveBeenCalledWith("/api/projects", {
      method: "POST",
  ...
  test("throws on a non-ok response", async () => {
    ...
    await expect(createProject({ name: "Roadmap" })).rejects.toThrow("failed to create project");
  });
});
...
describe("updateProject", () => {
  test("patches the project and encodes the id", async () => {
    ...
    await expect(updateProject("p 1/x", { name: "Renamed" })).resolves.toEqual(project);
  ...
  test("throws AuthExpiredError on 401", async () => {
    ...
    await expect(updateProject("p1", { name: "Renamed" })).rejects.toBeInstanceOf(AuthExpiredError);
  });
});
...
describe("setProjectStarred", () => {
  test.each([
    ...
    await expect(setProjectStarred("p1", starred)).resolves.toEqual({ ...project, starred });
    expect(fetchMock).toHaveBeenCalledWith(`/api/projects/p1/${action}`, { method: "POST" });
  });

  test("throws on a non-ok response", async () => {
    stubFetch(new Response("", { status: 500 }));

    await expect(setProjectStarred("p1", true)).rejects.toThrow("failed to update project");
  });
});

describe.each([
  ["archiveProject", archiveProject, "archive", "failed to archive project"],
  ["unarchiveProject", unarchiveProject, "unarchive", "failed to unarchive project"],
...
describe("project memory", () => {
  ...
  test("getProjectMemory throws on a non-ok response", async () => {
    ...
    await expect(getProjectMemory("p1")).rejects.toThrow("failed to load project memory");
  });
  ...
  test("refreshProjectMemory posts to the refresh endpoint", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith("/api/projects/p1/memory:refresh", { method: "POST" });
  });

  test("refreshProjectMemory throws on a non-ok response", async () => {
    stubFetch(new Response("", { status: 503 }));

    await expect(refreshProjectMemory("p1")).rejects.toThrow("failed to refresh project memory");
  });

  test("editProjectMemory sends the instruction", async () => {
    const fetchMock = stubFetch(Response.json(memory));

    await expect(editProjectMemory("p1", "forget the launch date")).resolves.toEqual(memory);
  ...
  test("editProjectMemory throws on a non-ok response", async () => {
    ...
    await expect(editProjectMemory("p1", "forget")).rejects.toThrow("failed to edit project memory");
  });
});
