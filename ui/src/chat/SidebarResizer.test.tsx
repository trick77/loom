...
describe("SidebarResizer", () => {
  ...
  // width to the default. The strip has to stay mostly on the sidebar's side.
  it("keeps all but a few pixels of the strip off the content", () => {
    render(<SidebarResizer />);
    const left = handle().style.left;
    // calc(var(--ui-sidebar-w) - Npx): N is how far the strip reaches back over the
    // sidebar, and 10px wide means 10 - N hangs over the content.
  ...
  it("does not reset on a mouse double-click", () => {
    ...
    expect(valuenow()).toBe(480); // the sidebar width survives the double-click
    expect(localStorage.getItem("loom:sidebar-width")).toBe("480");
  });
