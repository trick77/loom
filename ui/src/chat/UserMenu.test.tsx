...
describe("UserMenu", () => {
  ...
  it("expands an English/Deutsch picker and switches on selection", async () => {
    ...
    const onClose = vi.fn();
    render(
      <UserMenu onSettings={vi.fn()} onLogout={vi.fn()} onClose={onClose} />,
    );

    // Collapsed by default.
    expect(
      screen.queryByRole("menuitemradio", { name: "Deutsch (Deutschland)" }),
    ...
    fireEvent.click(
      screen.getByRole("menuitemradio", { name: "Deutsch (Deutschland)" }),
    );
  ...
  it("aligns icons with the first line of wrapping action text", () => {
    render(
      <UserMenu onSettings={vi.fn()} onLogout={vi.fn()} onClose={() => {}} />,
    );
