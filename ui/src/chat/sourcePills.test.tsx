...
describe("webSourceMap", () => {
  it("collects web citations (url + index) and ignores RAG doc citations", () => {
    ...
    expect(map?.get(1)).toEqual({ url: "https://truefoundry.com", label: "Truefoundry" });
    expect(map?.get(2)?.label).toBe("Modal");
  });

  it("returns undefined when there are no web citations", () => {
    expect(webSourceMap([{ documentId: "d1", filename: "a.pdf", snippet: "", score: 0.5 }])).toBeUndefined();
...
describe("ProseMarkdown inline source pills", () => {
  ...
  it("replaces [n] markers with clickable source pills linking to the URL", () => {
    render(<ProseMarkdown sources={sources}>{"TrueFoundry deploys models [1]. Modal runs Python [2]."}</ProseMarkdown>);
  ...
  it("leaves unknown markers as literal text", () => {
    render(<ProseMarkdown sources={sources}>{"An out-of-range marker [7] stays as text."}</ProseMarkdown>);
