...
export function assignDisplayNumbers(
  ...
  // losing all visibility into what was searched — and it would also be empty for
  // the mid-stream window before the first marker arrives, since sources are
  // delivered ahead of the deltas that cite them.
  if (ordered.length === 0) {
