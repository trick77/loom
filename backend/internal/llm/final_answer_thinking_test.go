...
// fields of the outbound chat-completion request.
func captureStreamRequest(t *testing.T, ctx context.Context) streamRequestFields {
	t.Helper()
...
func TestClient_StreamSuppressesThinkingAndWidensBudgetFromContext(t *testing.T) {
	...
	req := captureStreamRequest(t, ctx)
	if req.Thinking == nil || req.Thinking.Type != "disabled" {
		t.Fatalf("thinking = %+v, want {type:disabled}", req.Thinking)
	}
	if req.ReasoningEffort != "" {
		t.Fatalf("reasoning_effort = %q, want empty when thinking is suppressed", req.ReasoningEffort)
	}
