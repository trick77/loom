...
func TestMessagesPersistActivityTrace(t *testing.T) {
	...
	rawTrace := json.RawMessage(`[{"id":"reasoning-1","type":"reasoning","content":"I searched first.","status":"done"},{"id":"call_1","type":"tool","name":"search__web","status":"done","rawArguments":"{\"q\":\"lume\"}","rawOutput":"search result"}]`)
	message, err := store.AddMessageWithActivityTrace(ctx, userID, thread.ID, RoleAssistant, "I found Lume.", MessageTokenUsage{}, nil, rawTrace)
	if err != nil {
