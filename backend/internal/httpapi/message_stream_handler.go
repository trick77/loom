...
func (s *server) handleStreamMessage(w http.ResponseWriter, r *http.Request) {
	...
	// Persist the RAG + web-search citations so the answer's sources survive a
	// reload. Web sources gathered during the tool loop carry URL + [n] index; they
	// share the citations column with RAG doc citations (distinguished by url).
	allSources := knowledgeSources
	if webCitations := webSourceCitations(assistantResult.WebSources); len(webCitations) > 0 {
