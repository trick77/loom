...
TMP_DIR=$(mktemp -d "${TMPDIR:-/tmp}/spark-refresh-test.XXXXXX")
...
LOG_FILE=${SPARK_REFRESH_TEST_LOG:?}

printf '%s\n' "$*" >> "$LOG_FILE"

case "$*" in
  "compose -f compose.dev.yaml build spark")
...
PATH="$TMP_DIR/bin:$PATH" SPARK_REFRESH_TEST_LOG="$LOG_FILE" "$ROOT/hack/refresh.sh"
